...
class UniformVelocityCommand(CommandTerm):
  ...
  def create_gui(
    ...
    def _default_slider_value(min_val: float, max_val: float) -> float:
    ...
    with server.gui.add_folder(name.capitalize()):
      ...
      for label, value_range in axes:
        ...
        slider_limit = max(abs(min_val), abs(max_val), 0.1)
        max_input = server.gui.add_slider(
          f"Max {label}",
          initial_value=slider_limit,
          step=0.1,
          min=0.1,
          max=10.0,
        )
        slider = server.gui.add_slider(
          ...
          max=slider_limit,
          step=0.05,
          initial_value=_default_slider_value(min_val, max_val),
        )
