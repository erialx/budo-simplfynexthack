...
**Landed:** `safety_integration.py` and its tests have moved into
`NaVILA-Orca/src/navila_orca/safety_integration.py` /
`NaVILA-Orca/tests/test_safety_integration.py`, and `cli.py` now wires
`build_safe_runner()` in behind `--safety`/`--no-safety` and
`--veto-client {stub,anthropic}`. This folder is kept for `check_real_backend.py`
(the real-`MjlabGo2Backend` validation script) and the historical `ANNA.md` log
below, which documents everything as it stood before the move.

This folder was scratch space for **A's** current task in `docs/PLAN.md`
